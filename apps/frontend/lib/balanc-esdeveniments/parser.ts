...
/** Imports europeus: «1.069,00» / «1069,00» / «(1.069,00)»; o nombre ja numèric. */
function parseImportCel(val: unknown): number | null {
  ...
  // Coma = decimal europeu → treure milers (punts) i convertir coma a punt
  if (s.includes(",")) {
    s = s.replace(/\./g, "").replace(",", ".");
  } else if (/^\d{1,3}(\.\d{3})+$/.test(s)) {
    // Només milers sense decimals: 1.069 → 1069
    s = s.replace(/\./g, "");
  }
...
function sheetToMatrix(sheet: XLSX.WorkSheet): (string | number | null)[][] {
  // raw:true → cel·les numèriques com a number (evita «1.069,00» mal interpretat)
