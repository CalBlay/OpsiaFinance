...
export interface AjustarImportCentreInput {
  centreId: string;
  any: number;
  mes: number;
...
export async function ajustarImportCentreAction(input: AjustarImportCentreInput): Promise<Result> {
  ...
  if (!input.centreId) return ERR("Cal un centre.");
  ...
  const period = await db.period.upsert({
    ...
    update: {},
    create: { any: input.any, mes: input.mes, nom: `${MESOS_LLARGS[input.mes - 1]} ${input.any}` },
  });
