import {
  ...
  buildCostPersonalCuina,
} from "@/lib/external/cost-personal-estructura";
import { type NextRequest, NextResponse } from "next/server";

/**
 * API machine-to-machine per Cal Blay · Cost de serveis (Cuina central).
...
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await buildCostPersonalCuina(year, month);
    return NextResponse.json(data);
  } catch (err) {
    console.error("[external/cost-personal-cuina]", err);
